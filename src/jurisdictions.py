...
def classify_wa_tax_line(title: str) -> tuple[str, str]:
    """
    Classify a Washington state tax line.
    WA uses location-based taxation with state + local components.
    ...
    # State tax patterns
    if re.match(r"(?i)^washington\s+state", title) or re.match(r"(?i)^wa\s+state", title) or title.lower() == "state tax":
        return ("state", "Washington State")

    # Local tax - usually includes city/county name
    # Examples: "Seattle City Tax", "King County Tax", "Tacoma Local Tax"
    if re.search(r"(?i)city\s+tax", title):
        name = re.sub(r"(?i)\s+city\s+tax.*", "", title)
        return ("city", f"{name} City")

    if re.search(r"(?i)county\s+tax", title):
        name = re.sub(r"(?i)\s+county\s+tax.*", "", title)
        return ("county", f"{name} County")

    if re.search(r"(?i)local\s+tax", title):
        name = re.sub(r"(?i)\s+local\s+tax.*", "", title)
        return ("local", f"{name} Local")

    # RTA (Regional Transit Authority)
    if re.search(r"(?i)rta|transit", title):
        return ("transit", title)

    # Default - treat as local
    return ("local", title)


def aggregate_wa_by_jurisdiction(orders_with_tax_lines: list) -> dict:
    """
    Aggregate Washington tax collected by jurisdiction.
    ...
    results = {
        ...
        "county": defaultdict(lambda: {"amount": Decimal("0"), "count": 0}),
        "city": defaultdict(lambda: {"amount": Decimal("0"), "count": 0}),
        "local": defaultdict(lambda: {"amount": Decimal("0"), "count": 0}),
        "transit": defaultdict(lambda: {"amount": Decimal("0"), "count": 0}),
    }
    ...
    for order in orders_with_tax_lines:
        ...
        for tax_line in tax_lines:
            ...
            jur_type, jur_name = classify_wa_tax_line(title)
...
def generate_wa_jurisdiction_summary(jurisdiction_data: dict) -> dict:
    """Generate summary for Washington jurisdiction data."""
    summary = {
        "totals": {
            "state": 0.0,
            "county": 0.0,
...
def generate_wa_worksheet_html(
    ...
    """
    Generate a Washington-specific filing worksheet with location breakdown.
    """
...
<html>
<head>
    <title>Washington Combined Excise Tax Worksheet - {period_name}</title>
    <style>
        @media print {{
            body {{ margin: 0.5in; }}
...
<body>
    <div class="header">
        <h1>Washington Combined Excise Tax Worksheet</h1>
        <div class="subtitle">{period_name} | Local Sales Tax Breakdown by Location</div>
    </div>

    <div class="company-info">
        <strong>{company_info.get('name', '')}</strong><br>
        Federal EIN: {company_info.get('ein', '')} | WA UBI: {state_id or '________________'}<br>
        Period: {period_name}
    </div>
    ...
    <div class="note">
        <strong>Washington Location-Based Tax:</strong> Washington requires reporting by location code.
        Each city/county has a specific code. Use the DOR's Tax Rate Lookup tool to find codes for each location.
    </div>
    ...
    <div class="section">
        ...
        <table>
            ...
            <tr>
                <td>Local Taxes</td>
                <td class="amount">${totals['local']:,.2f}</td>
            </tr>
            <tr>
                <td>Transit (RTA) Taxes</td>
    ...
    <div class="section page-break">
        <h2>Detailed Location Breakdown</h2>

        <div class="note">
            Use this breakdown to complete your Combined Excise Tax Return.
            Look up location codes at: <a href="https://dor.wa.gov/taxes-rates/sales-use-tax/look-rate-location-code" target="_blank">WA DOR Tax Rate Lookup</a>
        </div>
"""
...
"""
    ...
    <div class="section no-print">
        ...
        <p>Use the location breakdown above to complete your Washington Combined Excise Tax Return.</p>
        <a href="https://dor.wa.gov/file-pay-taxes" target="_blank" class="portal-link">
            Open WA DOR Portal →
        </a>
    </div>
    ...
    <div class="section">
        ...
        <ul style="list-style: none; padding: 0;">
            <li>☐ Log into My DOR at dor.wa.gov</li>
            <li>☐ Enter gross sales: <strong>${total_sales:,.2f}</strong></li>
            <li>☐ Enter state sales tax: <strong>${totals['state']:,.2f}</strong></li>
            <li>☐ Enter local taxes by location code (see breakdown above)</li>
            <li>☐ Verify total: <strong>${totals['grand_total']:,.2f}</strong></li>
